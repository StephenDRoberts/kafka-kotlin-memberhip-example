...
* Note: I initially thought that when loading up two instances of the app then the partitions for both user-topic and state-store would be shared between the apps. I was coming across and InvalidStateStoreException with the reason seemingly being that the state-store in one app wasn't ready and that I needed to code defensively around this to wait until the store was ready (see [here](https://docs.confluent.io/3.2.0/streams/faq.html#handling-invalidstatestoreexception-the-state-store-may-have-migrated-to-another-instance)). However, I don't believe this is actually the case in my situation. This is because when a second app is set running, one application gets assigned to all paritions of a user-topic, and the other app gets assigned all partitions of the state-store. This means that if you query the state-store from the app that isn't assigned any partitions of the state-store, then an exception is thrown and doesn't resolve as there is no state-store waiting to ready itself. I\ve asked the question [here](https://stackoverflow.com/questions/65008721/kafka-topics-state-store-partitions-all-being-assigned-to-one-application-instan).
